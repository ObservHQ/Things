...
Use this command to the the respective template.
...
If you are server-snippet and is not allowed , try running the ingress-controller with `annotations-risk-level: "Critical"` in `ingress-nginx-controller` configmap.